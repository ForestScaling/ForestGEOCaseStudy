...
set.seed(101) 
sample = sample.split(rdata$pltID, SplitRatio = .75)
train = subset(rdata, sample == TRUE)%>%
  ungroup()
test  = subset(rdata, sample == FALSE)%>%
  ungroup()
...
rangermodel<-ranger(alpha~Max_Height+ STDAGE+ LON+LAT+Prop_Hardwood_to_Softwood+
       Total_precip+elevation+avgtemp,importance = "impurity",
       data= train%>%
         drop_na(),keep.inbag = TRUE,
       write.forest = TRUE, num.trees = 5000, verbose= TRUE)
print(rangermodel)
```
We then calculate how well the model predicts the test data
```{r}

# Predict on test set
predictions <- predict(rangermodel, data = test%>%
                         drop_na())

# Plot predicted vs actual values
ggplot(data = test%>%drop_na(), aes(x = predictions$predictions, y = alpha)) +
  geom_point() +
...
postResample(predictions$predictions, (test%>%drop_na())$alpha)
```
We aren't as interested in variable importance for now, but we can still calculate it if need be.
